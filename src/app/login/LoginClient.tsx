...
function LoginContent() {
  ...
  useEffect(() => {
    if (callbackError === "auth_callback_failed") {
      // Async state update to prevent react-hooks/set-state-in-effect warning
      setTimeout(() => {
        setErrorMsg("Falha na autenticação via provedor externo. Tente novamente.");
      }, 0);
    }
  }, [callbackError]);
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes("Invalid login credentials")) {
          ...
          setErrorMsg("E-mail não confirmado. Por favor, verifique sua caixa de entrada.");
        } else {
          setErrorMsg(error.message);
        }
      } else {
        setSuccessMsg("Autenticação realizada com sucesso! Redirecionando...");
        setTimeout(() => {
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      setErrorMsg("Erro ao iniciar login via GitHub.");
      setLoading(false);
      console.error(err);
    }
  };
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-950/80 backdrop-blur-xl p-8 shadow-2xl">
          ...
          <div className="text-center mb-8">
            <h1 className="font-display text-3xl font-extrabold text-white tracking-tight">
              Entrar no <span className="text-brand-red">PubPascal</span>
            </h1>
            <p className="text-sm text-slate-400 mt-2">
              Conecte-se para publicar e gerenciar seus pacotes Pascal
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                  className="w-full bg-slate-900 border border-slate-800 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-red/40 focus:border-brand-red/40 transition-all"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full inline-flex items-center justify-center rounded-lg bg-brand-red hover:bg-brand-red-dark text-white font-bold text-sm px-4 py-3 shadow-lg shadow-brand-red/20 transition-all hover:scale-[1.01] focus:outline-none focus:ring-2 focus:ring-brand-red/50 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100"
            >
              {loading ? (
