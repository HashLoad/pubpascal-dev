...
import { getAllPartners } from "@/utils/queries/admin-partners";
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="font-display text-2xl md:text-3xl font-extrabold text-white tracking-tight">
            Parceiros <span className="text-brand-red">Estratégicos</span>
          </h2>
          <p className="text-sm text-slate-400 mt-1 max-w-2xl">
            Gerencie a vitrine de parceiros do portal, status e ordem de exibição.
          </p>
        </div>
        <Link
      ...
      {rows.length === 0 ? (
        <div className="mx-auto max-w-xl text-center py-16">
          ...
          <p className="text-slate-400 leading-relaxed">
            Use <span className="font-mono text-brand-red">Novo parceiro</span> para começar a vitrine.
          </p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/60 backdrop-blur-md">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-850">
              {rows.map((row) => (
                <PartnersTableRow key={row.id} row={row} />
              ))}
            </tbody>
          </table>
        </div>
